...
import { useNavigate, BrowserRouter, Route, Routes, useLocation } from "react-router-dom";
...
function JobPost({ user }) {
    ...
    const [data, setData] = useState({
        ...
        grossFrom: 0,
        fullAddress: '',
        jobDescription: JSON.stringify(convertToRaw(jobDescription.getCurrentContent())),
    ...
    return (
        <>
            <Grid container sx={{ background: '#f1f2f6' }}>
                <Grid item xs={2}>
                    <HrSideBar name={user.user.username} companyName="Chưa cập nhật công ty" />
                </Grid>
                <Grid item xs={10}>
                    <Routes >
